...
class _MundoGianlucaScreenState extends State<MundoGianlucaScreen>
  ...
  String get _textoAtual {
    switch (_etapa) {
      case _Etapa.carregando:
        return 'Processando frequências harmônicas...';
      case _Etapa.bloqueado:
        return 'Acesso restrito ao Conservatório.';
      case _Etapa.chegada1:
        return 'Oh, olá vi"AU"jante, quem é você e o que lhe trás até meu conservatório?';
      case _Etapa.chegada2:
        return 'Humm, interessante... Terrasen é realmente um lugar de tir"AU"r o fôlego, mas o que lhe trás especificamente até a mim $_nomeJogador?';
      case _Etapa.dialogo1:
        return 'Ahhh, uma ajuda?! Bom então vamos fazer uma troca, eu te ajudo a ir para o próximo mundo e em troca você me faz um leve f"AU"vorzinho...';
      case _Etapa.dialogo2:
        return 'Ao tentar tocar o lendário acorde de EM7(9,13)/G#°m4add fui enfeitiç"AU"do pelo elfo da música. Ele me chamou de indisciplin"AU"do!';
      case _Etapa.dialogo3:
        return 'E me transformou em um cachorro! Para recuperar minha forma humana, preciso ensinar a harmonia natural para alguém. "AU"ceita ser meu aluno?';
      case _Etapa.espera:
        return 'Bom, nesse caso vamos esperar... Estarei aqui praticando minhas escalas. Me avise se mudar de ideia.';
      case _Etapa.positivo:
        return '"AU"migo! Fico feliz que decidiu me "AU"xiliar, vamos nessa!';
      case _Etapa.dialogo5:
        return 'A lição que vou te ensinar é a de ouvir o coração da música. Observe atentamente a melodia que irei tocar.';
      case _Etapa.introPiano:
        return 'Repita corretamente a melodia usando o piano. Se errar uma única nota, será necessário começar novamente.';
      case _Etapa.demonstrandoPiano:
        return 'Observe a sequência musical...';
      case _Etapa.tocandoPiano:
        return 'Agora é sua vez. Toque a melodia correta.';
      case _Etapa.reuniao1:
        return 'Incrível! Os fragmentos musicais ressoam perfeitamente. A partitura está completa.';
      case _Etapa.reuniao2:
        return 'Melodia. Harmonia. Ritmo. Três notas, um único acorde. 🎵';
      case _Etapa.finalHist:
        return 'O feitiço foi quebrado! Te agradeço por tudo, ótima sorte nessa sua jornada $_nomeJogador.';
    }
  }
    ...
    try {
      ...
      if (user == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Erro de autenticação. Entre novamente.'),
          ),
        );
      ...
      final progressoAnterior = data['terrasen'] ?? [];

      bool podeEntrar = false;

      if (progressoAnterior is List && progressoAnterior.isNotEmpty) {
  ...
  void _exibirTexto(String texto) {
    ...
    _timerTexto = Timer.periodic(const Duration(milliseconds: 30), (t) {
      if (!mounted) {
        t.cancel();
        return;
      }
  ...
  Future<void> _tocarSomLetra() async {
    ...
    try {
      ...
      await _typePlayer.play(AssetSource('audio/typewriter_click.mp3'));
  ...
  Future<void> _tocarNota(String nota) async {
    ...
    try {
      ...
      await _efeitosPlayer.play(AssetSource('audio/piano/$arquivo'));
    } catch (e) {
  ...
  Future<void> _tocarErro() async {
    ...
    try {
      ...
      await _efeitosPlayer.play(AssetSource('audio/piano/Fail_trumpet.mp3'));
    } catch (e) {
  ...
  Future<void> _tocarFurElise() async {
    try {
      ...
      if (_somAtivado) {
        await _musicaPlayer.play(AssetSource('audio/fur_elise.mp3'));
      }
    } catch (e) {
  ...
  Future<void> _pressionarNota(String nota) async {
    ...
    if (_entradaJogador.length == _sequenciaCorreta.length) {
      _podeTocar = false;
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          SizedBox.expand(
            child: Image.asset(
              'assets/images/IMAGEM_OFICIAL_CONSERVATORIO.png',
              fit: BoxFit.cover,
    ...
    switch (_etapa) {
      ...
      case _Etapa.chegada1:
        return _buildEscolhas([
          _OpcaoBtn(
            label:
                'Sou $_nomeJogador. Estou procurando o caminho para o próximo mundo.',
      ...
      case _Etapa.chegada2:
        return _buildEscolhas([
          _OpcaoBtn(
            label:
                'Preciso encontrar uma forma de seguir viagem. Talvez você possa me ajudar.',
  ...
  Widget _buildPiano() {
    ...
    return Padding(
      padding: const EdgeInsets.only(bottom: 260, left: 10, right: 10),
      child: SizedBox(
        height: 220,
        child: Row(
          children: notas.map((nota) {
            ...
            return Expanded(
              child: GestureDetector(
                onTap: _podeTocar ? () => _pressionarNota(nota) : null,
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: destaque ? Colors.amber : Colors.white,
